...
// legacyIngressSource is an implementation of Source for Kubernetes ingress objects.
...
type legacyIngressSource struct {
...
// NewLegacyIngressSource creates a new ingressLegacySource with the given config.
func NewLegacyIngressSource(ctx context.Context, kubeClient kubernetes.Interface, namespace, annotationFilter string, fqdnTemplate string, combineFqdnAnnotation bool, ignoreHostnameAnnotation bool, ignoreIngressTLSSpec bool, ignoreIngressRulesSpec bool, labelSelector labels.Selector) (Source, error) {
...
func (sc *legacyIngressSource) Endpoints(ctx context.Context) ([]*endpoint.Endpoint, error) {
	...
	for _, ing := range ingresses {
		...
		ingEndpoints := endpointsFromLegacyIngress(ing, sc.ignoreHostnameAnnotation, sc.ignoreIngressTLSSpec, sc.ignoreIngressRulesSpec)
...
func (sc *legacyIngressSource) endpointsFromTemplate(ing *extensionsv1beta1.Ingress) ([]*endpoint.Endpoint, error) {
	...
	if len(targets) == 0 {
		targets = targetsFromLegacyIngressStatus(ing.Status)
	}
...
func (sc *legacyIngressSource) filterByAnnotations(ingresses []*extensionsv1beta1.Ingress) ([]*extensionsv1beta1.Ingress, error) {
...
func (sc *legacyIngressSource) setResourceLabel(ingress *extensionsv1beta1.Ingress, endpoints []*endpoint.Endpoint) {
	for _, ep := range endpoints {
		ep.Labels[endpoint.ResourceLabelKey] = fmt.Sprintf("ingress/%s/%s", ingress.Namespace, ingress.Name)
	}
}

func (sc *legacyIngressSource) setDualstackLabel(ingress *extensionsv1beta1.Ingress, endpoints []*endpoint.Endpoint) {
...
// endpointsFromLegacyIngress extracts the endpoints from ingress object
func endpointsFromLegacyIngress(ing *extensionsv1beta1.Ingress, ignoreHostnameAnnotation bool, ignoreIngressTLSSpec bool, ignoreIngressRulesSpec bool) []*endpoint.Endpoint {
	ttl, err := getTTLFromAnnotations(ing.Annotations)
	if err != nil {
		log.Warn(err)
	}
...
func targetsFromLegacyIngressStatus(status extensionsv1beta1.IngressStatus) endpoint.Targets {
...
func (sc *legacyIngressSource) AddEventHandler(ctx context.Context, handler func()) {
